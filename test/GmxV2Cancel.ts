...
describe("GmxV2-Position", async () => {
  ...
  it("cancel order", async () => {
    ...
    await setBalance(user0.address, toWei("2000"))
    await weth.connect(user0).deposit({ value: toWei("1000") })
    await weth.connect(user0).approve(proxyFactory.address, toWei("1000"))
        ...
        proxyFactory.interface.encodeFunctionData("proxyFunctionCall", [
          {
            ...
            referralCode: ethers.utils.formatBytes32String("muxprotocol"),
            value: 0,
            proxyCallData: proxy.interface.encodeFunctionData("cancelOrder", [orders[i].key]),
          },
        ])
      )
    }

    var tx = await proxyFactory.connect(user0).multicall(data)
